...
void AHRS_ComputeAngles(void)
{
	...
	MadgwickQuaternionUpdate(-accelData.y, -accelData.x, accelData.z, gyroData.y,
			gyroData.x, -gyroData.z, magData.x,	magData.y, magData.z, AHRS_Angle); /*to update the orientation angles (pitch, roll, and yaw). Passes accelerometer, gyroscope, and magnetometer data as inputs to the filter.
